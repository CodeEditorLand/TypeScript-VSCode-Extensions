		...
		const listDiags = (diags: DiagnosticRelatedInformation[]) => {
			...
			diags.forEach((diag) => {
				...
				if (typeof diag === "string") {
					...
					li.textContent = ts.flattenDiagnosticMessageText(
						diag.messageText,
						"\n",
						4,
					);
				...
				Object.keys(node).forEach((field) => {
					...
					if (
						...
						properties.appendChild(
							renderManyChildren(field, value, depth),
						);
					} else if (
						typeof value === "object" &&
						"pos" in value &&
						...
						properties.appendChild(
							renderSingleChild(field, value, depth),
						);
					} else {
						properties.appendChild(
							renderLiteralField(field, value, info),
						);
					}
				});
			};
